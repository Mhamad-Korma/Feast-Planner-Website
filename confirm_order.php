...
//    get the last exchange rate
    $sql_rate = "SELECT rate FROM exchange_rate ORDER BY rate DESC LIMIT 1";
...
//    multiply the exchange rate by the total$ and update the totalLBP in the order table
    $totalLBP = $totalAmount*$exchange_rate;

    $sql = "UPDATE `order` SET order_total_lbp = ? WHERE order_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("di", $totalLBP, $orderId);
    $stmt->execute();
    $stmt->close();
